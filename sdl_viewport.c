 ...
 * source and destination are unmodified if true is returned
 */
 ...
 * This function expects positive (> 0) values for width and height of the object and the viewport or else
 * garbage values will be returned. This function is a lot cheaper than its "full" version and in contrast,
 * it does not return whether the object is in the viewport or not. Use sdl_inViewport() for that.
 * @param view the viewport
 * @param pos position and dimensions of drawable object
 * @param vdst the SDL_Rect structure describing where to draw the viewport to
 * @param dst destination SDL_Rect structure to be filled
 */
void sdl_getViewportDstRect(DRect *view, DRect *pos, SDL_Rect *vdst, SDL_Rect *dst) {
    // texture needs to be scaled accordingly if viewport and vdst dimensions mismatch;
    // if aspect ratios mismatch, then vscaleW != vscaleH, which means the texture is stretched
    const double vscaleW = vdst->w / view->w;
    const double vscaleH = vdst->h / view->h;

    dst->x = (int) (vscaleW * (pos->x - view->x) + vdst->x);
    dst->y = (int) (vscaleH * (pos->y - view->y) + vdst->y);
    dst->w = (int) (vscaleW * pos->w);
    dst->h = (int) (vscaleH * pos->h);
}
 ...
 * @param dst destination SDL_FRect structure to be filled
 */
void sdl_getViewportDstFRect(DRect *view, DRect *pos, SDL_Rect *vdst, SDL_FRect *dst) {
    // texture needs to be scaled accordingly if viewport and vdst dimensions mismatch;
    // if aspect ratios mismatch, then vscaleW != vscaleH, which means the texture is stretched
    const double vscaleW = vdst->w / view->w;
    const double vscaleH = vdst->h / view->h;

    dst->x = (float) (vscaleW * (pos->x - view->x) + vdst->x);
    dst->y = (float) (vscaleH * (pos->y - view->y) + vdst->y);
    dst->w = (float) (vscaleW * pos->w);
    dst->h = (float) (vscaleH * pos->h);
}
